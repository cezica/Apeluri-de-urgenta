...
std::string CerereIstoricDisp::_cerere()
{
    return std::string();
}
...
void CerereIstoricDisp::_cerere_ist_vechi(Ui::PrimaPag *ui)
{
    ...
    QString trimite = "4$" + user_dispecer + "$";
...
//elimiarea apelului din lista de asteptare-> adaugare in istoric
bool CerereIstoricDisp::_cerere_elim_apel(Ui::PrimaPag *ui,QString detalii)
{
    //trimitere catre server 5$username_dispecer$s_apel$string detalii$
    //din lista de asteptare preluam username-ul clientului
    QString rand = ui->lista_in_astp->currentItem()->text();
    QStringList impartire = rand.split(" - ");
    //impartire[0] e username-ul clientului
    //impartire[1] sf apelului
...
void CerereIstoricDisp::_cerere_istoric_apeluri(Ui::PrimaPag *ui)
{
    ...
    QString trimite = "6$" + user_dispecer + "$";
    Conexiune::getinstance()->trimite_cerere(trimite.toStdString());
