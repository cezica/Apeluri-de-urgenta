...
void PrimaPag::on_autentificare_clicked()
{
    ...
    if (tip == "client")
    {
        ...
        //istoric client
        CerereIstoricCl ci(username,ui);
        ci._cerere();
    }
    else if (tip == "dispecer")
    {
        ...
        d_u.set_ui(ui);
        d_u.set_tip_user(QString::fromStdString(tip));
        d_u._cerere();
        date_util = d_u;
    }
...
void PrimaPag::on_sch_parola_2_clicked()
{
    ...
    //ca o cerere de a vedea inregistrarile
    CerereInreg is_inregis = CerereInreg();
    is_inregis._cerere_inregistrari(ui);
    is_inreg = is_inregis;
...
void PrimaPag::on_inregistrare_2_clicked()
{
    ...
    if (c._cerere() == "v") ui->stackedWidget->setCurrentIndex(1);
    else
    {
...
void PrimaPag::on_but_stergere_clicked()
{
    ...
    bool rp=date_util._cerere_sterge_utl(user_selectat,username);
    if (rp == false)afisare_er("Eroare cerere de stergere!");
}
...
void PrimaPag::on_b_stergere_disp_clicked()
{
    ...
    if (raspuns == "x")
        afisare_er("Nu s-a putut sterge dispecerul!");
...
void PrimaPag::on_b_sterg_apel_clicked()
{
    ...
    if (is_admin._cerere_elim_apel(r) == false)
    {
...
void PrimaPag::on_but_stergere_2_clicked()
{
    ...
    date_util._cerere_sterge_utl(user_selectat,username);
}
...
void PrimaPag::on_aprob_inrg_clicked()
{
    ...
    if(is_inreg._cerere_aprobare_inreg(iterator,r)==false)
        afisare_er("Nu s-a putut aproba inregistrarea!");
    else
...
void PrimaPag::on_resping_inreg_clicked()
{
    ...
    is_inreg._cerere_respingere_inreg(iterator,r);

    if (is_inreg._cerere_respingere_inreg(iterator, r) == false)
        afisare_er("Nu s-a putut respinge inregistrarea!");
    else
...
void PrimaPag::on_resping_strg_clicked()
{
    ...
    if(is_stergeri._cerere_resping_stergere()==false)
        afisare_er("Nu s-a respins stergerea cu succes!");
    ui->lista_cereri_st->takeItem(r);
}
